...
struct Segment {
    ...
    init(px1: Float, py1: Float, px2: Float, py2: Float) {
        ...
        self.a = py2 - py1
        self.b = px1 - px2
        self.c = px2 * py1 - px1 * py2
        self.undefinded = (a == 0) && (b == 0) && (c == 0)
    }
    
    mutating func compare(other: Segment) -> SegmentRelation {
        if self.undefinded || other.undefinded {
            return .undefined
        }
        var grad1 = MAXFLOAT
        var grad2 = MAXFLOAT
...
class HachureIterator : IteratorProtocol {
    ...
    init(top: Float, bottom: Float, left: Float, right: Float, gap: Float, sinAngle: Float, cosAngle: Float, tanAngle: Float) {
        ...
        if abs(sinAngle) < MIN_LIMIT {
            ...
            self.pos = left - abs(self.deltaX)
            self.hGap = abs(gap / cosAngle)
            self.sLeft = Segment(px1: left, py1: bottom, px2: left, py2: top)
            self.sRight = Segment(px1: right, py1: bottom, px2: right, py2: top)
        }
    }
