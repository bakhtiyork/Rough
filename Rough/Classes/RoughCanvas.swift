...
@objc open class RoughCanvas: NSObject {
    ...
    @objc open func draw(context: CGContext, rect: CGRect) {
        for shape in paths {
            switch shape {
            case Shape.path(path: let path, options: let options):
                options.stroke.setStroke()
                path.lineWidth = CGFloat(options.strokeWidth)
                path.stroke()
            case Shape.fill(path: let path, options: let options):
                options.fill.setFill()
                path.fill()
            case Shape.hachure(path: let path, options: let options):
    ...
    @objc public func line(from: CGPoint, to: CGPoint, optionsCallback: OptionsCallback) {
        let customOptions = Options.copy(options: self.canvasOptions)
    ...
    @objc public func ellipse(center: CGPoint, width: Float, height: Float) {
        drawEllipse(center: center, width: width, height: height, options: self.canvasOptions)
    }
    ...
    @objc public func circle(center: CGPoint, radius: Float) {
        drawEllipse(center: center, width: 2 * radius, height: 2 * radius, options: self.canvasOptions)
    }
    ...
    func drawEllipse(center: CGPoint, width: Float, height: Float, options: Options) {
        ...
        if options.fill != UIColor.clear {
            if options.fillStyle == .hachure {
                ...
                paths.append(fill)
            } else if case .path(path: let path, options: let options) = shape {
    ...
    func drawArc(center: CGPoint, width: Float, height: Float, start: Float, stop: Float, closed: Bool, options: Options) {
        ...
        if options.fill != UIColor.clear {
            if options.fillStyle == .hachure {
                ...
                if case .path(path: let path, options: let options) = path {
                    let fill = Shape.fill(path: path, options: options)
                    paths.append(fill)
                }
            }
